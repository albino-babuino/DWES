...
<?php
    ...
    <p>
        ...
        <input type="number"  min="1" max="31" step="1"  id="dia" name="dia" value="{$dia}" />
    </p>
    <p>
        <label for="mes">Introduzca un número de mes [1..12]:</label>
        <input type="number"  min="1" max="12" step="1"  id="mes" name="mes" value="{$mes}" />
    </p>    
    <p>
        <label for="anno">Introduzca un número año [1..]:</label>
        <input type="number" min="1" step="1"  id="anno" name="anno" value="{$anno}" />
    </p>    
MARCA;
    $segundaVez = false;
    if ( !isset($_GET['dia']) ) {
    ...
    if ( !isset($_GET['dia']) || !isset($_GET['mes'])  || !isset($_GET['anno']))  {  // aunque el html se encarga de comprobar rango, no hace daño probar aquí
         if ($segundaVez) {
            echo <<<MARCA
